...
namespace DatabaseMigration.Core.Interpreters
{
    public class OracleBulkCopy : IDisposable
    {
        ...
        public async Task WriteToServerAsync(DataTable table)
        {
            ...
            if (UploadEverythingInSingleBatch)
                await WriteToServerInSingleBatchAsync(table);
            else
                await WriteToServerInMultipleBatchesAsync(table);
        }
        ...
        private async Task WriteToServerInSingleBatchAsync(DataTable table)
        {
            // Build the command string
            string commandText = BuildCommandText(table);

            await WriteSingleBatchOfDataAsync(table, 0, commandText, table.Rows.Count);
        }
        ...
        private async Task WriteToServerInMultipleBatchesAsync(DataTable table)
        {
            // Calculate number of batches
            int numBatchesRequired = (int)Math.Ceiling(table.Rows.Count / (double)BatchSize);
            ...
            for (int i = 0; i < numBatchesRequired; i++)
            {
                int skipOffset = i * BatchSize;
                int batchSize = Math.Min(BatchSize, table.Rows.Count - skipOffset);
                await WriteSingleBatchOfDataAsync(table, skipOffset, commandText, batchSize);
            }
        }
        ...
        private async Task WriteSingleBatchOfDataAsync(DataTable table, int skipOffset, string commandText, int batchSize)
        {
            // Get array of row data for all columns in the table
            List<OracleParameter> parameters = GetParameters(table, batchSize, skipOffset);
